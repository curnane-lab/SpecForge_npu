"""Shared speculative-decoding sampling helpers.

These small utilities are reused across SpecForge training, evaluation and
benchmark scripts so that sampling and rejection-residual logic stay consistent.
"""
...
def logits_to_probs(logits: torch.Tensor, temperature: float) -> torch.Tensor:
    ...
    For greedy decoding (temperature < 1e-5) returns a one-hot distribution
    instead of argmax so downstream code can still treat the result as probs.
    """
    if temperature < 1e-5:
...
def sample_from_probs(probs: torch.Tensor) -> torch.Tensor:
    """Sample one token per sequence position from a probability tensor."""
...
def sample_tokens(logits: torch.Tensor, temperature: float = 0.0) -> torch.Tensor:
    """Sample tokens from logits, supporting greedy decoding."""
    if temperature < 1e-5:
...
def gather_token_probs(probs: torch.Tensor, token_ids: torch.Tensor) -> torch.Tensor:
    """Gather the probability assigned to each token id."""
...
def sample_residual(target_probs: torch.Tensor, draft_probs: torch.Tensor) -> torch.Tensor:
    """Sample from the residual (target - draft)+ distribution.

    Used in speculative decoding to pick a replacement token when the draft
    token at a position is rejected.
    """
